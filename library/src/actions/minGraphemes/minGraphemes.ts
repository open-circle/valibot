...
> {
  return {
    ...
    '~run'(dataset, config) {
      if (dataset.typed) {
        // Hint: A grapheme consists of at least one code unit, so an input that
        // is not empty always contains at least one grapheme. This allows us to
        // skip the expensive segmentation for a requirement of 1 or less
