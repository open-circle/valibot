...
- Change `bytes`, `maxBytes`, `minBytes` and `notBytes` actions to count bytes with `TextEncoder.encodeInto` and a reused buffer instead of allocating a new byte array on every validation to improve performance (pull request #XXXX)
