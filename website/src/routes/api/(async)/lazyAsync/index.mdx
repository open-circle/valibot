...
> For recursive schemas with inferred input and output types, use <Link href="/api/recursiveAsync/">`recursiveAsync`</Link> instead.
...
> In most cases, <Link href="/api/recursiveAsync/">`recursiveAsync`</Link> is the better choice for recursive schemas because it can infer input and output types automatically.
