...
> If you need to validate the minimum number of user-perceived characters, use <Link href="../minGraphemes/">`minGraphemes`</Link> instead. If you need to count each Unicode code point as a single character (a supplementary code point occupies two UTF-16 code units in JavaScript), use <Link href="../minCodePoints/">`minCodePoints`</Link> instead.
...
<ApiList items={['minCodePoints', 'minGraphemes']} />
