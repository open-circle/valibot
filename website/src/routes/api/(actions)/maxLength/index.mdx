...
> If you need to validate the maximum number of user-perceived characters, use <Link href="../maxGraphemes/">`maxGraphemes`</Link> instead. If you need to count each Unicode code point as a single character (a supplementary code point occupies two UTF-16 code units in JavaScript), use <Link href="../maxCodePoints/">`maxCodePoints`</Link> instead.
...
<ApiList items={['maxCodePoints', 'maxGraphemes']} />
