...
> If you are looking for an equivalent of JavaScript's `.length` for minimum length checks, use <Link href="../minLength/">`minLength`</Link> instead. If you need to count each Unicode code point as a single character (a supplementary code point occupies two UTF-16 code units in JavaScript), use <Link href="../minCodePoints/">`minCodePoints`</Link> instead.
...
<ApiList items={['minCodePoints', 'minLength']} />
