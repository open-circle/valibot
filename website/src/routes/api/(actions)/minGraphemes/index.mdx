...
> If you are looking for an equivalent of JavaScript's `.length` for minimum length checks, use <Link href="../minLength/">`minLength`</Link> instead. If you need to count every Unicode code point including a supplementary code point, which requires 2 characters to be represented in JavaScript's string, as one character use <Link href="../minCodePoints/">`minCodePoints`</Link> instead.
