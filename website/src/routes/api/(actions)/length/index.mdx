...
> If you need to validate the number of user-perceived characters, use <Link href="../graphemes/">`graphemes`</Link> instead. If you need to count each Unicode code point as a single character (a supplementary code point occupies two UTF-16 code units in JavaScript), use <Link href="../codePoints/">`codePoints`</Link> instead.
...
<ApiList items={['codePoints', 'graphemes']} />
