...
> If you are looking for an equivalent of JavaScript's `.length` for maximum length checks, use <Link href="../maxLength/">`maxLength`</Link> instead. If you want to count every Unicode code point as one character for e.g. the password requirement by [NIST SP 800-63B](https://pages.nist.gov/800-63-4/sp800-63b.html#passwordver) or are looking for an equivalent of `LENGTH()` of open source RDBMSes like PostgreSQL, use <Link href="../maxCodePoints/">`maxCodePoints`</Link> instead.
