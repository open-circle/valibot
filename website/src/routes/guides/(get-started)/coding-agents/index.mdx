...
- `search_docs` searches the guides, API reference and blog posts and returns the most relevant pages
- `get_doc` reads a documentation page or blog post and returns its full content as Markdown
- `list_docs` lists all documentation pages and blog posts grouped by area and category
...
- [`llms-blog.txt`](/llms-blog.txt) contains the Markdown content of the blog posts

## Markdown for agents

We provide a Markdown version of every documentation page and blog post. You can access it by replacing the trailing slash (`/`) in the URL with `.md`. For example, `/guides/installation/` becomes `/guides/installation.md`.
