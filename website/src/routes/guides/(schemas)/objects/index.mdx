...
title: Objects
description: >-
  To validate objects with a schema, you can use `pojo`, `object` or `record`
  depending on how specific the validation should be.
contributors:
  - fabian-hiller
  - Mini-ghost
---
...
To validate objects with a schema, you can use <Link href="/api/pojo/">`pojo`</Link>, <Link href="/api/object/">`object`</Link> or <Link href="/api/record/">`record`</Link>. Use <Link href="/api/pojo/">`pojo`</Link> when you only need to validate that the input is a plain object, <Link href="/api/object/">`object`</Link> for an object with a specific shape and <Link href="/api/record/">`record`</Link> for objects with any number of uniform entries.
...
### Plain objects without entry validation

If you only need to validate that the input is a plain object and do not want to define or validate entries, use <Link href="/api/pojo/">`pojo`</Link>. Unlike <Link href="/api/object/">`object`</Link> and <Link href="/api/record/">`record`</Link>, it only checks the root type. It also accepts objects with a `null` prototype.

```ts
import * as v from 'valibot';

const PojoSchema = v.pojo();
```
