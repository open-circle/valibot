...
The same shape can also be built by hand, which is a good illustration of how `lazy` enables recursive schemas. Note that `jsonValue` is implemented directly and, unlike this hand-rolled composition, rejects non-finite numbers such as `Infinity` and `-Infinity`, detects circular references and rejects them with an issue instead of overflowing the call stack, and reports different issues:
