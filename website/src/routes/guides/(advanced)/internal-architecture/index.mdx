...
| `'~standard'` | `StandardProps` | Standard Schema v1 properties                                               |
...
Two things are important when creating a modified copy. First, wrap the new object with `_standardSchema` so that its `'~standard'.validate` function uses the modified schema. Spreading alone copies the original schema's validator. Second, capture the original schema in a closure rather than reading `this` in `'~run'`, so the original `'~run'` logic is called correctly.
